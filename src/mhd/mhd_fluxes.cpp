...
void MHD::CalculateFluxes(Driver *pdriver, int stage) {
  ...
  KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k, const int j) {
    ...
    auto e31 = e31_;
    auto e21 = e21_;
    if constexpr (rsolver_method_ == MHD_RSolver::llf) {
      LLF(member,eos,indcs,size,m,k,j,il,iu,IVX,wl,wr,bl,br,bx,flx1,e31,e21);
    } else if constexpr (rsolver_method_ == MHD_RSolver::hlld) {
      HLLD(member,eos,indcs,size,m,k,j,il,iu,IVX,wl,wr,bl,br,bx,flx1,e31,e21);
    } else if constexpr (rsolver_method_ == MHD_RSolver::llf_sr) {
      LLF_SR(member,eos,indcs,size,m,k,j,il,iu,IVX,wl,wr,bl,br,bx,flx1,e31,e21);
    }
    member.team_barrier();
    ...
    KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k) {
      ...
      for (int j=jl; j<=ju; ++j) {
        ...
        if (j>jl) {
          ...
          auto e32 = e32_;
          if constexpr (rsolver_method_ == MHD_RSolver::llf) {
            LLF(member,eos,indcs,size,m,k,j,is-1,ie+1,IVY,wl,wr,bl,br,by,flx2,e12,e32);
          } else if constexpr (rsolver_method_ == MHD_RSolver::hlld) {
            HLLD(member,eos,indcs,size,m,k,j,is-1,ie+1,IVY,wl,wr,bl,br,by,flx2,e12,e32);
          } else if constexpr (rsolver_method_ == MHD_RSolver::llf_sr) {
            LLF_SR(member,eos,indcs,size,m,k,j,is-1,ie+1,IVY,wl,wr,bl,br,by,flx2,e12,e32);
          }
          member.team_barrier();
        }
    ...
    KOKKOS_LAMBDA(TeamMember_t member, const int m, const int j) {
      ...
      for (int k=kl; k<=ku; ++k) {
        ...
        if (k>kl) {
          ...
          auto e13 = e13_;
          if constexpr (rsolver_method_ == MHD_RSolver::llf) {
            LLF(member,eos,indcs,size,m,k,j,is-1,ie+1,IVZ,wl,wr,bl,br,bz,flx3,e23,e13);
          } else if constexpr (rsolver_method_ == MHD_RSolver::hlld) {
            HLLD(member,eos,indcs,size,m,k,j,is-1,ie+1,IVZ,wl,wr,bl,br,bz,flx3,e23,e13);
          } else if constexpr (rsolver_method_ == MHD_RSolver::llf_sr) {
            LLF_SR(member,eos,indcs,size,m,k,j,is-1,ie+1,IVZ,wl,wr,bl,br,bz,flx3,e23,e13);
          }
          member.team_barrier();
        }
