...
//!
//! Athena-miniK (this version) is an even more simplified implmentation of the core
//! compute kernels in AthenaK, written for use with pyThena for demonstration of basic
//! concepts in fluid dynamics. It implements algorithms for Newtonian and special
//! relativistic hydro and MHD on a Cartesian grid with AMR.
//========================================================================================
  ...
  for (int i=1; i<argc; i++) {
    ...
    if (*argv[i] == '-'  && *(argv[i]+1) != '\0' && *(argv[i]+2) == '\0') {
      ...
      switch(*(argv[i]+1)) {
        ...
        default:
          if (global_variable::my_rank == 0) {
            std::cout << "Athena-miniK" << std::endl;
