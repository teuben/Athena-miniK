...
void Hydro::CalculateFluxes(Driver *pdriver, int stage) {
  ...
  KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k, const int j) {
    ...
    if constexpr (rsolver_method_ == Hydro_RSolver::llf) {
      LLF(member, eos, indcs, size, m, k, j, il, iu, IVX, wl, wr, flx1);
    } else if constexpr (rsolver_method_ == Hydro_RSolver::hllc) {
      HLLC(member, eos, indcs, size, m, k, j, il, iu, IVX, wl, wr, flx1);
    } else if constexpr (rsolver_method_ == Hydro_RSolver::llf_sr) {
      LLF_SR(member, eos, indcs, size, m, k, j, il, iu, IVX, wl, wr, flx1);
    } else if constexpr (rsolver_method_ == Hydro_RSolver::hllc_sr) {
      HLLC_SR(member, eos, indcs, size, m, k, j, il, iu, IVX, wl, wr, flx1);
    }
    member.team_barrier();
    ...
    KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k) {
      ...
      for (int j=jl; j<=ju; ++j) {
        ...
        if (j>jl) {
          ...
          auto flx2 = flx2_;
          if constexpr (rsolver_method_ == Hydro_RSolver::llf) {
            LLF(member, eos, indcs, size, m, k, j, il, iu, IVY, wl, wr, flx2);
          } else if constexpr (rsolver_method_ == Hydro_RSolver::hllc) {
            HLLC(member, eos, indcs, size, m, k, j, il, iu, IVY, wl, wr, flx2);
          } else if constexpr (rsolver_method_ == Hydro_RSolver::llf_sr) {
            LLF_SR(member, eos, indcs, size, m, k, j, il, iu, IVY, wl, wr, flx2);
          } else if constexpr (rsolver_method_ == Hydro_RSolver::hllc_sr) {
            HLLC_SR(member, eos, indcs, size, m, k, j, il, iu, IVY, wl, wr, flx2);
          }
          member.team_barrier();
        }
    ...
    KOKKOS_LAMBDA(TeamMember_t member, const int m, const int j) {
      ...
      for (int k=kl; k<=ku; ++k) {
        ...
        if (k>kl) {
          ...
          auto flx3 = flx3_;
          if constexpr (rsolver_method_ == Hydro_RSolver::llf) {
            LLF(member, eos, indcs, size, m, k, j, il, iu, IVZ, wl, wr, flx3);
          } else if constexpr (rsolver_method_ == Hydro_RSolver::hllc) {
            HLLC(member, eos, indcs, size, m, k, j, il, iu, IVZ, wl, wr, flx3);
          } else if constexpr (rsolver_method_ == Hydro_RSolver::llf_sr) {
            LLF_SR(member, eos, indcs, size, m, k, j, il, iu, IVZ, wl, wr, flx3);
          } else if constexpr (rsolver_method_ == Hydro_RSolver::hllc_sr) {
            HLLC_SR(member, eos, indcs, size, m, k, j, il, iu, IVZ, wl, wr, flx3);
          }
          member.team_barrier();
        }
